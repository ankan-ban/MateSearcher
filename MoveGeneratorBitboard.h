...
// not used
    ...
    CUDA_CALLABLE_MEMBER static CPU_FORCE_INLINE uint64 findPinnedPieces (uint64 myKing, uint64 enemyBishops, uint64 enemyRooks, uint64 allPieces, uint8 kingIndex)
    {
...
#if USE_TEMPLATE_CHANCE_OPT == 1
    template <uint8 chance>
    CUDA_CALLABLE_MEMBER static int isCheckMate(HexaBitBoardPosition *pos)
#else
    CUDA_CALLABLE_MEMBER static int isCheckMate(HexaBitBoardPosition *pos, uint8 chance)
#endif
    {
        uint64 allPawns     = pos->pawns & RANKS2TO7;    // get rid of game state variables
        ...
        if (threatened & (pos->kings & myPieces))
        {
            uint32 moves = 0;
#if USE_TEMPLATE_CHANCE_OPT == 1
            moves = countMovesOutOfCheck<chance>(pos, allPawns, allPieces, myPieces, enemyPieces,
                                                 pinned, threatened, kingIndex);
#else
            moves = countMovesOutOfCheck(pos, allPawns, allPieces, myPieces, enemyPieces,
                                         pinned, threatened, kingIndex, chance);
#endif
            if (moves == 0)
                return 1;
...
#if USE_TEMPLATE_CHANCE_OPT == 1
    template <uint8 chance>
    CUDA_CALLABLE_MEMBER static uint32 countMoves (HexaBitBoardPosition *pos, bool *isInCheck)
#else
    CUDA_CALLABLE_MEMBER static uint32 countMoves (HexaBitBoardPosition *pos, uint8 chance, bool *isInCheck)
#endif
    {
    ...
    template<uint8 chance>
    static int generateCaptures(const ExpandedBitBoard *bb, CMove *captures)
    {
        int nMoves = 0;
